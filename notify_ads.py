...
def notify_ads():
    ...
    if notified_ads_count > 0:
        loop.run_until_complete(
            send(bot, CHAT_ID, f"Total new ads sent: {notified_ads_count} [{ads_data['previous_ads_count']} ads sent earlier!]"))
    else:
        loop.run_until_complete(
            send(bot, CHAT_ID, f"No new ads found! [{ads_data['previous_ads_count']} ads sent earlier!]"))
